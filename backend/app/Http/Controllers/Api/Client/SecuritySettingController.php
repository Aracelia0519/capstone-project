...
class SecuritySettingController extends Controller
{
    ...
    public function sendRecoveryOtp(Request $request)
    {
        ...
        // If enabling, use the new recovery email. If disabling, send to main email.
        ...
        Log::info("SECURITY OTP for User {$user->id} ({$targetEmail}): {$otp}");
    ...
    public function verifyRecoveryOtp(Request $request)
    {
        ...
        if ($cachedData['action'] === 'enable') {
            ...
            $user->save(); // Intelephense error is fixed via the DocBlock above
