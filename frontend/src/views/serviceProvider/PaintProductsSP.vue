...
// Icons
import { 
  ShoppingBag, Clock, Package, CheckCircle2, RefreshCw, ChevronDown, ChevronUp, Image as ImageIcon, Truck, XCircle, FileText, AlertCircle, MapPin, CreditCard, Loader2, X, Star, Camera, Upload, Navigation, AlertTriangle, Banknote, Menu, Info, RotateCcw, Send, Paperclip, Search
...
const initWebSockets = (userId) => {
  ...
  // NEW: Listen to private channel for Real-Time Order Status Updates
  window.Echo.private(`provider.${userId}.orders`)
    .listen('.order.updated', (e) => {
       fetchOrders(true) // Silent fetch so it doesn't interrupt user UI
       
       // Silently refresh the return request info if the chat drawer happens to be open
       if (isReturnChatOpen.value && activeReturnItem.value) {
         refreshReturnChat(activeReturnItem.value.id)
       }

       toast.info('Order Status Updated', {
           description: 'One of your orders has been updated by the Distributor or Rider.',
...
const refreshReturnChat = async (itemId) => {
    try {
        const res = await api.get(`/service-provider/orders/items/${itemId}/return-chat`)
...
onMounted(() => {
  fetchOrders()
  if (props.user?.id) initWebSockets(props.user.id)
})
...
onUnmounted(() => {
  ...
  if (window.Echo && props.user) {
      ...
      window.Echo.leave(`provider.${props.user.id}.orders`)
  }
})
...
const openPickUpTracking = async (order) => {
  ...
  removePaymentProof()
  try {
    const response = await api.get(`/service-provider/orders/${order.id}/pickup-details`)
...
const requestLocationAndStartTracking = () => {
  ...
  navigator.geolocation.getCurrentPosition(
    (position) => {
      ...
      nextTick(() => { initMap(); startTracking() })
    },
    (error) => { locationError.value = "Location access denied. Please enable location to verify pickup." },
...
const initMap = () => {
  ...
  const userIcon = L.divIcon({
    html: `<div class="h-10 w-10 bg-slate-900 rounded-full border-2 border-blue-500 flex items-center justify-center shadow-[0_0_15px_rgba(59,130,246,0.7)] relative">
      <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="text-blue-400">
        <path d="M14 18V6a2 2 0 0 0-2-2H4a2 2 0 0 0-2 2v11a1 1 0 0 0 1 1h2"/><path d="M15 18H9"/><path d="M19 18h2a1 1 0 0 0 1-1v-3.65a1 1 0 0 0-.22-.624l-3.48-4.35A1 1 0 0 0 17.52 8H14"/><circle cx="17" cy="18" r="2"/><circle cx="7" cy="18" r="2"/>
      </svg>
      <div class="absolute -bottom-2 left-1/2 -translate-x-1/2 w-0 h-0 border-l-[6px] border-l-transparent border-r-[6px] border-r-transparent border-t-[8px] border-t-blue-500"></div>
    </div>`,
    className: 'bg-transparent', iconSize: [40, 48], iconAnchor: [20, 48]
  })
  ...
  if (pickUpDetails.value.distributor_lat && pickUpDetails.value.distributor_lng) {
    const targetIcon = L.divIcon({
      html: `<div class="h-8 w-8 bg-teal-500 rounded-full border-2 border-slate-900 flex items-center justify-center shadow-[0_0_20px_rgba(20,184,166,0.6)]">
    ...
    targetMarker.bindPopup(`<b class="text-slate-900">Distributor Shop</b><br><span class="text-slate-600">${pickUpDetails.value.distributor_address}</span>`)
  }
  drawRoute(true)
}
...
const drawRoute = async (force = false) => {
  ...
  try {
    ...
    if (data.code === 'Ok' && data.routes.length > 0) {
      ...
      routeLine = L.polyline(data.routes[0].geometry.coordinates.map(c => [c[1], c[0]]), { color: color, weight: 5, opacity: 0.8, lineCap: 'round', lineJoin: 'round' }).addTo(leafletMap)
    } else drawFallbackRoute(color)
  } catch { drawFallbackRoute(color) }
}
...
const submitPickUp = async () => {
  ...
  try {
    const response = await api.post(`/service-provider/orders/${trackedOrder.value.id}/pickup-submit`, formData, {
...
// --- Utils ---
const formatCurrency = (amount) => new Intl.NumberFormat('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(Number(amount || 0))
...
const getStatusConfig = (status) => {
  const configs = {
    pending: { color: 'bg-yellow-900/50 text-yellow-400', text: 'Pending' },
    confirmed: { color: 'bg-blue-900/50 text-blue-400', text: 'Confirmed' },
    prepared: { color: 'bg-indigo-900/50 text-indigo-400', text: 'Prepared' },
    ready_for_pickup: { color: 'bg-teal-900/50 text-teal-400', text: 'Ready For Pick-Up' },
    shipped: { color: 'bg-purple-900/50 text-purple-400', text: 'Shipped' },
    delivered: { color: 'bg-green-900/50 text-green-400', text: 'Delivered' },
    cancelled: { color: 'bg-red-900/50 text-red-400', text: 'Cancelled' }
  }
  return configs[status.toLowerCase()] || { color: 'bg-slate-800 text-slate-400', text: status }
}
...
const getProgressData = (status) => {
  switch (status.toLowerCase()) {
    ...
    default: return { value: 0, text: '', color: '[&>div]:bg-slate-500' }
  }
}
...
const openImageInNewTab = (url) => { if (url) window.open(url, '_blank') }
const refreshOrders = () => { isRefreshing.value = true; fetchOrders() }
const loadMoreOrders = () => { displayedOrders.value += 5 }
const toggleOrderDetails = (orderId) => {
  const order = orders.value.find(o => o.id === orderId)
  if (order) order.expanded = !order.expanded
}
const viewOrderDetails = (order) => { selectedOrder.value = order; isDetailsModalOpen.value = true }
const cancelOrder = (orderId) => { if (!props.user) { isAuthAlertOpen.value = true; return }; toast.info('Cancellation requested.') }
const reorderItems = (orderId) => { if (!props.user) { isAuthAlertOpen.value = true; return }; toast.success('Items added to cart!') }
const trackOrder = (orderId) => { if (!props.user) { isAuthAlertOpen.value = true; return }; toast.info(`Tracking Order #${orderId}...`) }
...
const handleReturnProofUpload = (e) => {
  ...
  if (target.files && target.files[0]) {
    returnProofFile.value = target.files[0]; returnProofPreview.value = URL.createObjectURL(target.files[0])
  }
}
const removeReturnProof = () => {
  returnProofFile.value = null
  if (returnProofPreview.value) { URL.revokeObjectURL(returnProofPreview.value); returnProofPreview.value = null }
}
const handleTrackingProofUpload = (e) => { const target = e.target; if (target.files && target.files[0]) trackingProofFile.value = target.files[0] }
const handlePickupUpload = (e) => {
  const target = e.target
  if (target.files && target.files[0]) { pickupProofFile.value = target.files[0]; pickupProofPreview.value = URL.createObjectURL(target.files[0]) }
}
...
const handlePaymentUpload = (e) => {
  const target = e.target
  if (target.files && target.files[0]) { paymentProofFile.value = target.files[0]; paymentProofPreview.value = URL.createObjectURL(target.files[0]) }
}
...
const handleChatImageUpload = (e) => {
  const target = e.target
  if (target.files && target.files[0]) { chatImageFile.value = target.files[0]; chatImagePreview.value = URL.createObjectURL(target.files[0]) }
}
const removeChatImage = () => {
  chatImageFile.value = null
  if (chatImagePreview.value) { URL.revokeObjectURL(chatImagePreview.value); chatImagePreview.value = null }
  const fileInput = document.getElementById('chatImg')
  if (fileInput) fileInput.value = ''
}
const scrollToBottom = () => {
  nextTick(() => { const container = document.getElementById('returnChatContainer'); if (container) container.scrollTop = container.scrollHeight })
}

const openReturnChat = async (item) => {
  if (!props.user) { isAuthAlertOpen.value = true; return }
  isDetailsModalOpen.value = false
  activeReturnItem.value = item; isReturnChatOpen.value = true; returnRequest.value = null; returnMessages.value = []
  selectedReturnReason.value = ''; customReturnReason.value = ''; removeReturnProof(); removeChatImage()

  try {
    const res = await api.get(`/service-provider/orders/items/${item.id}/return-chat`)
    if (res.data.success && res.data.request) {
      returnRequest.value = res.data.request; returnMessages.value = res.data.messages || []; scrollToBottom()
    }
  } catch (error) { console.error(error) }
}
const closeReturnChat = () => { isReturnChatOpen.value = false }
...
const submitInitialReturn = async () => {
  ...
  try {
    const res = await api.post('/service-provider/orders/return-request', fd, { headers: { 'Content-Type': 'multipart/form-data' } })
    if (res.data.success) {
      toast.success('Return request submitted')
      returnRequest.value = res.data.request
      const chatRes = await api.get(`/service-provider/orders/items/${activeReturnItem.value.id}/return-chat`)
...
const sendChatMessage = async () => {
  ...
  try {
    const res = await api.post(`/service-provider/orders/returns/${returnRequest.value.id}/message`, fd, { headers: { 'Content-Type': 'multipart/form-data' } })
    if (res.data.success) {
      returnMessages.value.push(res.data.data)
      chatMessage.value = ''; removeChatImage(); scrollToBottom()
    }
  } catch (error) { toast.error('Failed to send message') } finally { isSendingMessage.value = false }
}
...
const submitTrackingInfo = async () => {
  ...
  try {
    const res = await api.post(`/service-provider/orders/returns/${returnRequest.value.id}/tracking`, fd, { headers: { 'Content-Type': 'multipart/form-data' } })
    if (res.data.success) {
      toast.success('Tracking info submitted')
      returnRequest.value = res.data.request
      returnMessages.value.push(res.data.message)
      trackingNumber.value = ''; trackingProofFile.value = null; scrollToBottom()
      fetchOrders(true)
    }
  } catch (error) { toast.error('Failed to submit tracking') } finally { isSubmittingTracking.value = false }
}
</script>
  ...
  <div class="min-h-screen  font-sans text-slate-200">
      ...
      <div class="container mx-auto px-4 md:px-8 py-6">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p class="text-slate-400 mt-1 ml-[52px]">Track your paint supplies and pick-ups</p>
          </div>
          ...
          <div class="flex items-center space-x-3 w-full md:w-auto">
            ...
            <Button variant="outline" size="icon" @click="refreshOrders" :disabled="isRefreshing" class="bg-slate-950 border-slate-800 text-slate-400 hover:text-blue-400 hover:bg-slate-900 shrink-0">
    ...
    <div class="container mx-auto px-4 md:px-8 py-8 max-w-7xl">
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-8">
        <Card class="bg-slate-900/50 backdrop-blur-sm border-slate-800 rounded-2xl overflow-hidden hover:border-slate-700 transition-colors">
          <CardContent class="p-6 flex items-center justify-between">
            ...
            <div class="w-12 h-12 bg-blue-900/30 rounded-full flex items-center justify-center shrink-0 border border-blue-800/50">
        ...
        <Card class="bg-slate-900/50 backdrop-blur-sm border-slate-800 rounded-2xl overflow-hidden hover:border-slate-700 transition-colors">
          <CardContent class="p-6 flex items-center justify-between">
            ...
            <div class="w-12 h-12 bg-yellow-900/30 rounded-full flex items-center justify-center shrink-0 border border-yellow-800/50">
        ...
        <Card class="bg-slate-900/50 backdrop-blur-sm border-slate-800 rounded-2xl overflow-hidden hover:border-slate-700 transition-colors">
          <CardContent class="p-6 flex items-center justify-between">
            <div>
              ...
              <p class="text-3xl font-bold text-indigo-400 mt-1">{{ processingCount }}</p>
            </div>
            <div class="w-12 h-12 bg-indigo-900/30 rounded-full flex items-center justify-center shrink-0 border border-indigo-800/50">
        ...
        <Card class="bg-slate-900/50 backdrop-blur-sm border-slate-800 rounded-2xl overflow-hidden hover:border-slate-700 transition-colors">
          <CardContent class="p-6 flex items-center justify-between">
            <div>
              ...
              <p class="text-3xl font-bold text-green-400 mt-1">{{ completedCount }}</p>
            </div>
            <div class="w-12 h-12 bg-green-900/30 rounded-full flex items-center justify-center shrink-0 border border-green-800/50">
      ...
      <div class="space-y-6">
        ...
        <Card v-else-if="filteredOrders.length === 0" class="bg-slate-900/50 border-slate-800 rounded-3xl p-12 text-center">
          <div class="w-20 h-20 mx-auto mb-6 bg-slate-950 rounded-full flex items-center justify-center text-slate-600 border border-slate-800">
          ...
          <div class="flex items-center justify-center gap-4">
            <Button v-if="statusFilter !== 'all'" @click="statusFilter = 'all'" variant="outline" class="border-slate-700 bg-slate-950 text-slate-300 hover:bg-slate-800">Clear Filters</Button>
          </div>
        </Card>

        <Card v-else v-for="order in filteredOrders" :key="order.id" class="group relative bg-gradient-to-br from-slate-900 to-slate-950 border-slate-800 rounded-2xl overflow-hidden transition-all duration-300 hover:border-blue-500/50 hover:shadow-2xl hover:shadow-blue-900/10">
          <div class="p-5 md:p-6 border-b border-slate-800/50">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div class="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
                <h3 class="text-lg font-bold text-white group-hover:text-blue-300 transition-colors">
                  Order <span class="text-blue-400">{{ order.order_number }}</span>
                </h3>
                <div class="flex items-center gap-3">
                  ...
                  <p class="text-sm text-slate-500 sm:hidden">{{ formatDate(order.created_at) }}</p>
                </div>
                <p class="text-sm text-slate-500 hidden sm:block">Placed on {{ formatDate(order.created_at) }}</p>
              </div>
              <div class="flex justify-between items-center md:block md:text-right border-t border-slate-800 md:border-t-0 pt-4 md:pt-0">
                <p class="text-sm text-slate-500 md:hidden">Total Amount</p>
                <div>
                  <div class="text-2xl font-black text-white">₱{{ formatCurrency(order.grand_total) }}</div>
                  <p class="text-sm text-slate-500 text-right">{{ order.items.length }} item(s)</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div class="mt-6 pt-6 border-t border-slate-800/50">
              <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                <div class="flex flex-wrap gap-4 text-sm">
                  <div class="bg-slate-950 px-3 py-1.5 rounded-lg flex items-center gap-2 border border-slate-800 shadow-inner">
                    <CreditCard class="w-4 h-4 text-slate-500" />
                    <span class="font-medium text-slate-300 uppercase">{{ order.payment_method }}</span>
                  </div>
                  <div class="bg-slate-950 px-3 py-1.5 rounded-lg flex items-center gap-2 border border-slate-800 shadow-inner">
                ...
                <div class="flex flex-wrap gap-2 w-full lg:w-auto">
                  ...
                  <Button v-if="order.status === 'pending'" variant="outline" @click="cancelOrder(order.id)" class="border-red-900/50 bg-slate-950 text-red-400 hover:bg-red-950/30 flex-1 sm:flex-none">
                    <XCircle class="w-4 h-4 mr-2" /> Cancel
                  </Button>
                  <Button v-if="(order.payment_method === 'pick-up' || order.payment_method === 'pickup') && (order.status === 'ready_for_pickup' || order.status === 'prepared')" @click="openPickUpTracking(order)" class="bg-teal-600 hover:bg-teal-700 text-white shadow-lg shadow-teal-900/20 border-0 flex-1 sm:flex-none">
                    <MapPin class="w-4 h-4 mr-2" /> View Pick-Up
                  </Button>
                  <Button v-else-if="order.status === 'shipped'" @click="trackOrder(order.id)" class="bg-purple-600 hover:bg-purple-700 text-white shadow-lg shadow-purple-900/20 border-0 flex-1 sm:flex-none">
                    <Truck class="w-4 h-4 mr-2" /> Track
                  </Button>
                </div>
              </div>
              
              <div v-if="order.status !== 'cancelled'" class="mt-6 bg-slate-950 rounded-xl p-4 border border-slate-800">
                <div class="mb-2 flex justify-between text-sm">
                  <span class="font-medium text-slate-400 flex items-center gap-2">
                    <AlertCircle class="w-4 h-4 text-slate-500" /> Status
                  </span>
                  <span class="font-semibold text-white">{{ getProgressData(order.status).text }}</span>
                </div>
                <Progress :model-value="getProgressData(order.status).value" :class="['h-2.5 bg-slate-800', getProgressData(order.status).color]" />
                <div class="flex justify-between mt-3 text-xs font-medium text-slate-500">
                  <span :class="{'text-blue-400': getProgressData(order.status).value >= 25}">Placed</span>
                  <span :class="{'text-blue-400': getProgressData(order.status).value >= 50}">Processing</span>
                  <span :class="{
                    'text-teal-400': getProgressData(order.status).value >= 75 && (order.payment_method === 'pick-up' || order.payment_method === 'pickup'),
                    'text-purple-400': getProgressData(order.status).value >= 75 && order.payment_method !== 'pick-up' && order.payment_method !== 'pickup'
                  }">
                    {{ (order.payment_method === 'pick-up' || order.payment_method === 'pickup') ? 'Ready' : 'Shipped' }}
                  </span>
                  <span :class="{'text-green-400': getProgressData(order.status).value === 100}">Delivered</span>
                </div>
              </div>
            </div>
          </div>
        </Card>
        <div v-if="filteredOrders.length > 0 && displayedOrders < orders.length" class="mt-8 text-center border-t border-slate-800 pt-8">
          <Button @click="loadMoreOrders" variant="outline" class="bg-slate-950 border-slate-800 text-slate-400 hover:bg-slate-900 w-full sm:w-auto px-8">Load More Orders</Button>
        </div>
      </div>
    </div>
    ...
    <Teleport to="body">
      <transition enter-active-class="transition duration-300 ease-out" enter-from-class="opacity-0 scale-95" enter-to-class="opacity-100 scale-100" leave-active-class="transition duration-200 ease-in" leave-from-class="opacity-100 scale-100" leave-to-class="opacity-0 scale-95">
        <div v-if="isPickUpTrackingActive" class="fixed inset-0 z-[9999] flex flex-col overflow-hidden bg-slate-950 text-slate-200 font-sans">
          <div v-if="!locationGranted" class="absolute inset-0 z-50 flex items-center justify-center backdrop-blur-md bg-slate-950/60 p-4">
            <div class="flex flex-col items-center justify-center flex-1 w-full max-w-md mx-auto text-center space-y-6">
              ...
              <div v-else class="relative">
                ...
                <div class="bg-slate-900/80 p-6 rounded-full relative shadow-inner border border-teal-500/30">
              ...
              <div class="space-y-2">
                ...
                <p class="text-slate-400 text-sm md:text-base px-4">To trace your path to the shop and validate your pick-up, GPS access is needed.</p>
              </div>
              ...
              <div class="flex gap-4 w-full sm:w-auto">
                <Button @click="closePickUpTracking" variant="outline" size="lg" class="bg-slate-900 text-white hover:bg-slate-800 border-slate-700">Cancel</Button>
                <Button @click="requestLocationAndStartTracking" size="lg" :disabled="isFetchingPickUp" class="flex-1 bg-teal-600 hover:bg-teal-700 text-white border-0 shadow-lg shadow-teal-900/20">
          ...
          <div v-if="locationGranted" class="absolute inset-0 z-10 pointer-events-none flex flex-col justify-between">
            <div class="p-4 md:p-6 pt-12 md:pt-6 flex justify-between items-start w-full">
              <div class="flex flex-col gap-2 pointer-events-auto max-w-[65%] sm:max-w-sm">
                <div class="bg-slate-900/80 backdrop-blur-md border border-slate-800 rounded-full py-1.5 px-3 shadow-lg flex items-center gap-2 w-max">
                  <div class="relative flex h-2.5 w-2.5 shrink-0">
                    <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-teal-400 opacity-75"></span>
                    <span class="relative inline-flex rounded-full h-2.5 w-2.5 bg-teal-500"></span>
                  </div>
                  <span class="text-xs font-medium text-slate-200 truncate font-mono">{{ currentCoordsDisplay }}</span>
                </div>
                <div class="bg-slate-900/80 backdrop-blur-md border border-slate-800 rounded-xl p-3 shadow-lg flex items-center gap-3">
                  ...
                  <div class="min-w-0">
                    ...
                    <p class="text-xs text-slate-400 truncate">{{ trackedOrder?.order_number }}</p>
                  </div>
                </div>
              </div>
              <div class="pointer-events-auto flex flex-col gap-2 items-end">
                <Button @click="closePickUpTracking" size="icon" variant="outline" class="bg-slate-900/90 backdrop-blur-md border-slate-700 text-white hover:bg-slate-800 shadow-xl h-12 w-12 rounded-full mb-2">
                  <X class="w-6 h-6" />
                </Button>
                <Button @click="isMapDrawerOpen = !isMapDrawerOpen" size="icon" variant="outline" class="bg-slate-900/90 backdrop-blur-md border-slate-700 text-white hover:bg-slate-800 shadow-xl h-12 w-12 rounded-full">
            ...
            <div class="bg-slate-950/95 border-t border-slate-800 backdrop-blur-xl pointer-events-auto w-full transition-transform duration-300 ease-in-out flex flex-col rounded-t-3xl shadow-[0_-20px_50px_rgba(0,0,0,0.5)] absolute bottom-0 left-0 right-0 z-20 h-[85vh] md:h-[70vh]" :style="{ transform: isMapDrawerOpen ? 'translateY(0)' : 'translateY(100%)' }">
              <div class="pt-6 pb-2 px-4 flex justify-between items-center border-b border-slate-800 mx-4 md:mx-6 shrink-0">
                <h2 class="text-lg font-bold text-white tracking-tight flex items-center gap-2">
                  <MapPin class="h-5 w-5 text-teal-500" /> Shop Location & Handover
                </h2>
                <Button variant="ghost" size="icon" @click="isMapDrawerOpen = false" class="text-slate-400 hover:text-white hover:bg-slate-800 rounded-full h-8 w-8">
                  <X class="w-4 h-4" />
                </Button>
              </div>
                ...
                <div class="space-y-6 pb-8">
                  <div class="flex justify-between items-start">
                    <div>
                      ...
                      <p class="text-sm text-slate-400 mt-1 max-w-sm">{{ pickUpDetails.distributor_address || 'Address unavailable' }}</p>
                    </div>
                  ...
                  <div class="bg-slate-900/40 rounded-2xl p-4 border border-slate-800">
                    ...
                    <div class="rounded-xl overflow-hidden bg-slate-950 h-40 relative flex items-center justify-center border border-slate-800">
                  ...
                  <div class="space-y-5">
                    <div>
                      ...
                      <div class="border-2 border-dashed border-slate-700 rounded-2xl flex flex-col items-center justify-center p-6 transition-colors relative" :class="pickupProofPreview ? 'bg-transparent' : 'bg-slate-900/50 hover:bg-slate-900'">
                        ...
                        <div v-if="!pickupProofPreview" class="text-center pointer-events-none">
                          <Camera class="h-8 w-8 text-slate-500 mx-auto mb-3" />
                          <p class="text-sm font-medium text-slate-300">Tap to snap Package</p>
                        </div>
                        <div v-else class="relative w-full flex justify-center">
                          <img :src="pickupProofPreview" class="max-h-56 rounded-xl border border-slate-700 object-cover shadow-lg" />
                    ...
                    <Button @click="submitPickUp" :disabled="!isWithinRange || !pickupProofFile || !paymentProofFile || isSubmittingPickUp" class="w-full bg-teal-600 hover:bg-teal-700 text-white shadow-lg shadow-teal-900/20 mt-4 rounded-xl h-14 text-lg font-semibold border-0" size="lg">
    ...
    <Dialog v-model:open="isDetailsModalOpen">
      <DialogContent class="sm:max-w-200 md:max-w-237.5 lg:max-w-275 max-h-[90vh] overflow-y-auto w-[95vw] p-0 gap-0 rounded-2xl bg-slate-950 border-slate-800 shadow-2xl">
        <div class="bg-slate-900 p-6 md:p-8 border-b border-slate-800 sticky top-0 z-10 flex justify-between items-start gap-4">
          ...
          <Button variant="ghost" size="icon" @click="isDetailsModalOpen = false" class="shrink-0 rounded-full hover:bg-slate-800 text-slate-400">
            <X class="w-5 h-5" />
          </Button>
        </div>
        <div class="p-6 md:p-8">
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2 space-y-6">
              <div class="bg-slate-900 rounded-xl border border-slate-800 overflow-hidden shadow-sm">
            ...
            <div class="lg:col-span-1 space-y-6">
              <div class="bg-slate-900 rounded-xl border border-slate-800 shadow-sm overflow-hidden">
                <div class="p-4 bg-slate-950 border-b border-slate-800">
                  <h4 class="font-bold text-white">Order Summary</h4>
                </div>
                <div class="p-5 space-y-4 text-sm font-medium">
                  <div class="flex justify-between text-slate-400">
                    <span>Subtotal</span>
                    <span class="text-slate-200">₱{{ formatCurrency(selectedOrder?.total_amount) }}</span>
                  </div>
                  <div class="flex justify-between text-slate-400">
                    <span>Shipping Fee</span>
                    <span class="text-slate-200">₱{{ formatCurrency(selectedOrder?.shipping_fee) }}</span>
                  </div>
                  <div class="pt-4 border-t border-dashed border-slate-800">
                    <div class="flex justify-between items-end">
                      <span class="text-base font-bold text-white">Grand Total</span>
                      <span class="text-2xl font-black text-blue-400">₱{{ formatCurrency(selectedOrder?.grand_total) }}</span>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <DialogContent class="sm:max-w-125 p-6 rounded-2xl bg-slate-950 border-slate-800 shadow-2xl w-[95vw] mx-auto">
        <DialogHeader class="mb-4">
          ...
          <DialogDescription class="text-slate-400 mt-2">
            How was your experience with <span class="font-bold text-slate-200">{{ reviewForm.product_name }}</span>?
          </DialogDescription>
        </DialogHeader>
        <div class="space-y-6">
          <div>
    ...
    <Teleport to="body">
      <div v-if="isReturnChatOpen" class="fixed inset-0 flex justify-end pointer-events-auto" style="z-index: 99999;">
        <div class="absolute inset-0 bg-slate-950/60 backdrop-blur-sm" @click.stop="closeReturnChat"></div>
        ...
        <transition enter-active-class="transition-transform duration-300 ease-out" enter-from-class="translate-x-full" enter-to-class="translate-x-0" leave-active-class="transition-transform duration-200 ease-in" leave-from-class="translate-x-0" leave-to-class="translate-x-full">
          <div v-if="isReturnChatOpen" class="relative w-full md:w-112.5 bg-slate-950 h-full shadow-2xl flex flex-col z-10 border-l border-slate-800">
            <div class="p-4 border-b border-slate-800 flex justify-between items-center bg-slate-900/50">
              <div>
                <h3 class="font-bold text-lg text-white">Return Item</h3>
                <p class="text-xs text-slate-400">{{ activeReturnItem?.product?.name }}</p>
              </div>
              <Button variant="ghost" size="icon" @click.stop.prevent="closeReturnChat" class="text-slate-400 hover:bg-slate-800 hover:text-white">
                <X class="w-5 h-5" />
              </Button>
            </div>

            <div v-if="!returnRequest" class="p-6 overflow-y-auto flex-1 space-y-5">
              <Alert class="bg-blue-900/20 text-blue-400 border-blue-900/50">
                <Info class="w-4 h-4" />
                <AlertDescription>Please provide a valid reason and clear photo proof to request a return.</AlertDescription>
              </Alert>
              <div>
                <label class="block text-sm font-bold text-slate-300 mb-2">Reason for Return</label>
                <Select v-model="selectedReturnReason">
                  <SelectTrigger class="w-full bg-slate-900 border-slate-800 text-slate-200 rounded-xl h-12">
                    <SelectValue placeholder="Select a reason" />
                  </SelectTrigger>
                  <SelectContent class="bg-slate-900 border-slate-800 text-slate-200 z-[100000]">
              ...
              <div v-if="selectedReturnReason === 'Others'">
                <label class="block text-sm font-bold text-slate-300 mb-2">Please specify</label>
                <textarea v-model="customReturnReason" rows="3" class="w-full p-3 rounded-xl border border-slate-800 bg-slate-900 text-slate-200 resize-none outline-none focus:ring-2 focus:ring-blue-500" placeholder="Type your reason here..."></textarea>
              </div>

              <div>
                <label class="block text-sm font-bold text-slate-300 mb-2">Photo Proof</label>
                <div class="border-2 border-dashed border-slate-700 rounded-xl p-6 text-center hover:bg-slate-900/50 cursor-pointer relative" :class="{'bg-transparent': returnProofPreview}">
                  ...
                  <div v-else class="relative inline-block">
                    <img :src="returnProofPreview" class="max-h-40 rounded-lg shadow-sm border border-slate-700" />
              ...
              <Button @click="submitInitialReturn" :disabled="isSubmittingReturn || !selectedReturnReason || (selectedReturnReason === 'Others' && !customReturnReason) || !returnProofFile" class="w-full bg-blue-600 border-0 hover:bg-blue-700 text-white font-bold h-12 rounded-xl">
                <Loader2 v-if="isSubmittingReturn" class="w-5 h-5 mr-2 animate-spin" /> Submit Return Request
              </Button>
            </div>

            <div v-else class="flex flex-col flex-1 overflow-hidden bg-slate-950">
              <div class="p-3 text-center text-sm font-semibold text-white shadow-sm shrink-0" :class="{ 'bg-yellow-600': returnRequest.status === 'pending', 'bg-green-600': returnRequest.status === 'approved', 'bg-red-600': returnRequest.status === 'rejected', 'bg-purple-600': returnRequest.status === 'shipped', 'bg-teal-600': returnRequest.status === 'completed' }">
              ...
              <div class="flex-1 overflow-y-auto p-4 space-y-4" id="returnChatContainer">
                <div v-for="msg in returnMessages" :key="msg.id" class="flex flex-col" :class="msg.sender_id === user?.id ? 'items-end' : 'items-start'">
                  <div v-if="msg.type === 'system'" class="w-full text-center my-4">
                    <span class="bg-slate-800 text-slate-300 text-xs px-3 py-1 rounded-full">{{ msg.message }}</span>
                    <img v-if="msg.file_path" :src="getFullImageUrl(msg.file_path)" class="max-w-50 mt-2 rounded-lg mx-auto border border-slate-700 shadow-sm" />
                  </div>
                  <div v-else class="max-w-[80%] rounded-2xl p-3 shadow-sm" :class="msg.sender_id === user?.id ? 'bg-blue-600 text-white rounded-br-none' : 'bg-slate-900 border border-slate-800 text-slate-200 rounded-bl-none'">
                    <p v-if="msg.type === 'text'" class="text-sm whitespace-pre-wrap">{{ msg.message }}</p>
                    <div v-else-if="msg.type === 'image'">
                      <img :src="getFullImageUrl(msg.file_path)" class="rounded-lg max-w-full cursor-pointer hover:opacity-90 border border-slate-700" @click="openImageInNewTab(getFullImageUrl(msg.file_path))" />
                      <p v-if="msg.message" class="text-sm mt-2">{{ msg.message }}</p>
                    </div>
                  </div>
                  <span class="text-[10px] text-slate-500 mt-1">{{ formatDate(msg.created_at) }}</span>
                </div>
              </div>

              <div v-if="returnRequest.status === 'approved'" class="p-4 bg-slate-900 border-t border-slate-800 shrink-0 space-y-3">
                <p class="text-xs font-bold text-green-400 uppercase tracking-wider">Submit Tracking Info</p>
                <input v-model="trackingNumber" type="text" placeholder="Tracking Number" class="w-full text-sm p-2.5 rounded-lg border border-slate-700 bg-slate-950 text-slate-200 outline-none focus:border-blue-500" />
                <div class="flex items-center gap-2">
                  <input type="file" id="trackImg" accept="image/*" class="hidden" @change="handleTrackingProofUpload" />
                  <label for="trackImg" class="cursor-pointer px-3 py-2 bg-slate-800 hover:bg-slate-700 rounded-lg text-sm font-medium flex-1 text-center truncate border border-slate-700 text-slate-300">
                    {{ trackingProofFile ? trackingProofFile.name : 'Upload Receipt Photo' }}
                  </label>
                  <Button @click="submitTrackingInfo" :disabled="!trackingNumber || !trackingProofFile || isSubmittingTracking" class="bg-green-600 border-0 hover:bg-green-700 text-white shrink-0">
                    <Send class="w-4 h-4" />
                  </Button>
                </div>
              </div>

              <div v-if="chatImagePreview" class="px-4 py-3 bg-slate-900/80 border-t border-slate-800 shrink-0">
                <div class="relative inline-block">
                  <img :src="chatImagePreview" class="h-20 w-20 object-cover rounded-xl shadow-md border border-slate-700" />
              ...
              <div v-if="['pending', 'approved', 'shipped'].includes(returnRequest.status)" class="p-3 bg-slate-900 border-t border-slate-800 shrink-0 flex items-end gap-2">
                <input type="file" id="chatImg" accept="image/*" class="hidden" @change="handleChatImageUpload" />
                <label for="chatImg" class="cursor-pointer p-3 rounded-full text-slate-400 hover:bg-slate-800 shrink-0 transition-colors" :class="{'text-blue-400 bg-blue-900/30': chatImageFile}">
                  <Paperclip class="w-5 h-5" />
                </label>
                <div class="flex-1 bg-slate-950 rounded-2xl border border-slate-800 focus-within:border-blue-500 flex items-center overflow-hidden transition-colors">
                  <input v-model="chatMessage" @keyup.enter="sendChatMessage" type="text" placeholder="Type a message..." class="w-full bg-transparent text-slate-200 border-none focus:ring-0 text-sm px-4 py-3 outline-none" />
                </div>
                <Button @click="sendChatMessage" :disabled="isSendingMessage || (!chatMessage.trim() && !chatImageFile)" size="icon" class="rounded-full h-11 w-11 shrink-0 bg-blue-600 hover:bg-blue-700 border-0 text-white shadow-md">
                  <Send class="w-4 h-4 ml-0.5" />
                </Button>
              </div>
              <div v-else class="p-3 text-center text-xs text-slate-500 bg-slate-900 border-t border-slate-800 shrink-0">
