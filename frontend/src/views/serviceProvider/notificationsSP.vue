<template>
  <div class="p-4 md:p-8 font-sans selection:bg-amber-100 selection:text-amber-900">
    <div class="max-w-8xl mx-auto space-y-6">
        ...
        <div class="flex items-center gap-5">
          ...
          <div>
            ...
            <p class="text-sm text-slate-500 mt-1 font-medium">
              You have <strong class="text-amber-600 dark:text-amber-400">{{ unreadCount }}</strong> unread messages.
            </p>
          </div>
        </div>
        <Button 
      ...
      <div class="bg-white dark:bg-slate-900 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-200 dark:border-slate-800 overflow-hidden relative">
        ...
        <div v-if="isLoading" class="p-24 flex flex-col items-center justify-center text-slate-400">
          <Loader2 class="w-12 h-12 animate-spin mb-4 text-amber-500" />
        ...
        <div v-else class="divide-y divide-slate-100 dark:divide-slate-800/50">
          ...
          >
            ...
            <div 
              ...
              class="absolute left-0 top-0 bottom-0 w-1.5 bg-amber-500 shadow-[0_0_10px_rgba(245,158,11,0.5)]"
            ></div>
            ...
            <div class="flex-1 space-y-2 min-w-0">
              <div class="flex flex-col md:flex-row md:items-start justify-between gap-2">
                ...
                <div class="flex items-center gap-2 shrink-0 md:pt-1">
                  <span class="text-xs font-bold uppercase tracking-wider" :class="!notification.is_read ? 'text-amber-600' : 'text-slate-400'">
                    {{ formatTimeAgo(notification.created_at) }}
                  </span>
                  <span v-if="!notification.is_read" class="w-2.5 h-2.5 rounded-full bg-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.6)]"></span>
                </div>
              </div>
              ...
              <div v-if="notification.attachment" class="mt-4 inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white dark:bg-slate-900 text-xs font-bold text-amber-600 dark:text-amber-400 border border-amber-100 dark:border-amber-900 shadow-sm transition-transform group-hover:scale-105">
...
// Fetch Notifications from DB using SP endpoints
...
const markAsRead = async (notification) => {
  ...
  try {
    await api.put(`/service-provider/notifications/${notification.id}/read`)
  } catch (error) {
    // Revert if failed
...
const markAllAsRead = async () => {
  ...
  try {
    await api.post('/service-provider/notifications/mark-all-read')
  } catch (error) {
...
const getIcon = (type) => {
  ...
  if (t.includes('order') || t.includes('job') || t.includes('procurement')) return Package
  ...
  if (t.includes('partner') || t.includes('client') || t.includes('distributor')) return Users
...
const getIconColor = (type) => {
  const t = type.toLowerCase()
  if (t.includes('order') || t.includes('job') || t.includes('procurement')) return 'bg-indigo-100 text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-400'
