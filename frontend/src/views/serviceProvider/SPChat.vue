<template>
  <div class="h-[calc(100dvh-4rem)] md:h-[calc(100vh-6rem)] min-h-[500px] md:min-h-[600px] p-0 md:p-4 text-slate-200">
              ...
              <div v-else-if="message.type === 'official_deal'" class="bg-gradient-to-br from-indigo-900/50 to-cyan-900/50 border border-cyan-500/50 rounded-xl p-4 shadow-lg shadow-cyan-900/20 w-full max-w-sm sm:max-w-md" :class="message.sender === 'me' ? 'rounded-tr-sm' : 'rounded-tl-sm'">
                 ...
                 <div class="space-y-2 text-xs text-slate-200 bg-slate-900/50 p-3 rounded-lg border border-slate-800 shadow-inner">
                    ...
                    <div v-if="message.payload?.colors" class="flex flex-col gap-1 mt-2">
                       <span class="text-slate-400">Requested Color(s):</span>
                       <Badge class="bg-slate-800 border border-slate-600 text-slate-300 self-start text-[10px]">{{ JSON.parse(message.payload.colors) }}</Badge>
                    </div>
                 </div>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent class="bg-slate-800 border-slate-700 text-slate-200 mb-2 ml-2 w-56 rounded-xl shadow-xl shadow-black/50">
                 ...
                 <div class="p-1">
                   ...
                   <DropdownMenuItem class="cursor-pointer focus:bg-slate-700 py-2.5 rounded-lg mt-1" @click="openProductShareModal">
                      <Package class="w-4 h-4 mr-2 text-emerald-400" /> 
                      <span class="font-medium text-sm">Share Product</span>
                   </DropdownMenuItem>
                 </div>
              </DropdownMenuContent>
            </DropdownMenu>
    ...
    <Dialog v-model:open="showProductShareModal">
      <DialogContent class="bg-slate-900 border-slate-800 text-slate-200 max-w-md rounded-2xl">
        <DialogTitle class="flex items-center gap-2 text-emerald-400 font-bold"><Package class="w-5 h-5"/> My Inventory</DialogTitle>
        <div class="space-y-3 max-h-[400px] overflow-y-auto custom-scrollbar p-1 mt-2">
           <div v-for="item in inventoryProducts" :key="item.id" @click="sendProduct(item)" class="bg-slate-950 border border-slate-800 rounded-xl p-3 cursor-pointer hover:border-emerald-500 transition-all flex items-center gap-4">
              <img :src="getPayloadImage(item.image)" class="w-16 h-16 rounded-lg object-cover border border-slate-700 shrink-0" @error="(e) => e.target.src='https://via.placeholder.com/150'" />
              <div class="flex-1 min-w-0">
                 <p class="text-sm font-bold truncate text-white mb-1">{{ item.name }}</p>
                 <p class="text-xs text-slate-400 mb-1"><span class="font-medium text-slate-500">Stock:</span> {{ item.quantity }}</p>
                 <p class="text-sm text-emerald-400 font-black">₱{{ item.price }}</p>
              </div>
           </div>
           <div v-if="inventoryProducts.length === 0" class="text-center text-slate-500 text-sm py-4">Inventory empty.</div>
        </div>
        <Button variant="ghost" class="mt-4 w-full text-slate-400 hover:text-white" @click="showProductShareModal = false">Cancel</Button>
      </DialogContent>
    </Dialog>
...
const showProductShareModal = ref(false)
...
// To deploy to Hostinger, just comment out the Local line and uncomment the Production line!
// =========================================================================
...
// Safely strip /api from the end if it exists, then append /storage/
...
const confirmEditMessage = async () => {
   ...
   try {
      const res = await api.put(`/service-provider/chat/messages/${editingMessageId.value}`, { message: editMessageText.value })
      if (res.data.success) {
...
const deleteMessage = async (id) => {
   try {
      const res = await api.delete(`/service-provider/chat/messages/${id}`)
      if (res.data.success) {
...
const uploadImage = async (e) => {
   ...
   try {
      const res = await api.post('/service-provider/chat/send-image', formData, {
         headers: { 'Content-Type': 'multipart/form-data' }
      })
      if(res.data.success) {
         let uploadedUrl = res.data.message.message;
         if (uploadedUrl && !uploadedUrl.startsWith('http')) {
             uploadedUrl = baseStorageUrl + uploadedUrl.replace(/^\/+/, '').replace(/^storage\//, '')
         }
...
const openColorShareModal = async () => {
   try {
      const res = await api.get('/service-provider/colors')
      if (res.data.success) {
...
// Products - STRICT AND EXACT LOGIC
const openProductShareModal = async () => {
   try {
      const res = await api.get('/service-provider/inventory')
      if (res.data.success) {
         const dataArray = res.data.data || []
         
         inventoryProducts.value = dataArray.map(item => {
            ...
            return {
               ...
               type: prod.type || 'N/A',
               color_code: prod.color_code || null,
               description: prod.description || 'No description available',
               brand: prod.brand || 'No brand',
               image: prod.image_url || 'https://via.placeholder.com/150'
            }
         })
         showProductShareModal.value = true
      }
   } catch(err) {
...
const sendProduct = async (product) => {
   ...
   const success = await submitMessageToDb('product_share', 'Shared a product', payload)
   if(success) showProductShareModal.value = false
}

const sendRequestSummary = async () => {
  const req = activeContact.value.requestContext || {}
  const summaryPayload = {
    title: req.service_offering?.title || activeContact.value.service_title || 'Custom Service',
  ...
  await submitMessageToDb('request_summary', 'Service Request Summary', summaryPayload)
}
