    ...
    <div class="max-w-8xl mx-auto space-y-6">
      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-200 dark:border-slate-800">
        <div class="flex items-center gap-5">
          ...
          <div>
            <h1 class="text-2xl md:text-3xl font-black text-slate-900 dark:text-white tracking-tight">Notifications</h1>
            <p class="text-sm text-slate-500 mt-1 font-medium">
              You have <strong class="text-blue-600 dark:text-blue-400">{{ unreadCount }}</strong> unread messages.
            </p>
          </div>
        </div>
        <Button 
      ...
      <div class="bg-white dark:bg-slate-900 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-200 dark:border-slate-800 overflow-hidden relative">
        ...
        <div v-if="isLoading" class="p-24 flex flex-col items-center justify-center text-slate-400">
          ...
          <p class="font-bold text-lg">Fetching notifications...</p>
        </div>
        ...
        <div v-else-if="notifications.length === 0" class="p-24 flex flex-col items-center justify-center text-slate-400">
          ...
          <p class="text-base mt-2 text-slate-400">No new notifications at this time.</p>
        </div>
        ...
        <div v-else class="divide-y divide-slate-100 dark:divide-slate-800/50">
          <div 
            ...
            :class="!notification.is_read ? 'bg-blue-50/50 dark:bg-blue-900/10' : ''"
            @click="markAsRead(notification)"
          >
            <!-- Left border indicator for unread -->
            <div 
              v-if="!notification.is_read" 
              class="absolute left-0 top-0 bottom-0 w-1.5 bg-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.5)]"
            ></div>
            ...
            <div class="flex-1 space-y-2 min-w-0">
              <div class="flex flex-col md:flex-row md:items-start justify-between gap-2">
                ...
                <div class="flex items-center gap-2 shrink-0 md:pt-1">
                  <span class="text-xs font-bold uppercase tracking-wider" :class="!notification.is_read ? 'text-blue-600' : 'text-slate-400'">
                    {{ formatTimeAgo(notification.created_at) }}
                  </span>
                  <span v-if="!notification.is_read" class="w-2.5 h-2.5 rounded-full bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.6)]"></span>
                </div>
              </div>
...
const getIcon = (type) => {
  ...
  if (t.includes('alert') || t.includes('warning') || t.includes('report')) return ShieldAlert
  if (t.includes('success') || t.includes('approved')) return CheckCircle2
  if (t.includes('message') || t.includes('chat')) return MessageSquare
  if (t.includes('partner')) return Users
  return Info
}

const getIconColor = (type) => {
  const t = type.toLowerCase()
  if (t.includes('order')) return 'bg-indigo-100 text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-400'
  if (t.includes('alert') || t.includes('warning') || t.includes('report')) return 'bg-rose-100 text-rose-600 dark:bg-rose-900/30 dark:text-rose-400'
  if (t.includes('success') || t.includes('approved')) return 'bg-emerald-100 text-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-400'
