<template>
  <div class="max-w-8xl px-4 py-8 mx-auto sm:px-6 lg:px-8">
    ...
    <div class="bg-white border border-gray-100 rounded-3xl shadow-xl shadow-gray-200/50 relative overflow-hidden">
      ...
      <div class="absolute top-0 right-0 -mt-10 -mr-10 w-40 h-40 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-full blur-3xl pointer-events-none"></div>
      ...
      <div class="p-6 sm:p-8 relative z-10">
        <div v-if="loading" class="flex flex-col items-center justify-center py-12">
          ...
          <p class="text-gray-500 font-bold animate-pulse">Encrypting connection...</p>
        </div>
        ...
        <div v-else class="space-y-5">
          ...
          >
            <div class="flex items-start gap-4 w-full sm:w-5/6">
              <!-- Dynamic Icon -->
              ...
              <div class="space-y-1 mt-0.5">
                <label class="text-base font-bold text-gray-900 group-hover:text-indigo-900 transition-colors cursor-pointer">
                  {{ item.title }}
                </label>
                <p class="text-sm text-gray-500 font-medium leading-relaxed pr-4">
                  {{ item.description }}
                </p>
              </div>
            </div>
            ...
            <!-- Modern Switch -->
            <div class="flex sm:justify-end shrink-0 pl-16 sm:pl-0">
              <button 
                type="button" 
                role="switch" 
    ...
    <Teleport to="body">
      ...
      >
        <div v-if="activeModal !== null" class="fixed inset-0 z-[100] bg-gray-900/60 backdrop-blur-sm flex items-center justify-center p-4 sm:p-6">
          ...
          >
            <div class="w-full max-w-lg bg-white shadow-2xl rounded-3xl max-h-[90vh] overflow-y-auto ring-1 ring-black/5 relative">
              
              <!-- Close Button (Absolute) -->
              <button @click="closeModal" class="absolute top-5 right-5 p-2 bg-gray-50 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-full transition-colors focus:outline-none">
              ...
              <div class="p-6 sm:p-8">
                ...
                <div v-if="activeModal === 'standard'">
                  <div class="w-14 h-14 bg-amber-100 text-amber-500 rounded-2xl flex items-center justify-center mb-5">
                  ...
                  <p class="text-base text-gray-500 font-medium mt-2 leading-relaxed" v-if="pendingField">
                ...
                <div v-if="activeModal === 'recovery_email'">
                  <div class="w-14 h-14 bg-blue-100 text-blue-600 rounded-2xl flex items-center justify-center mb-5">
                  ...
                  <p class="text-sm text-gray-500 font-medium mt-2">Enter the email address you wish to use as a secure backup.</p>
                  
                  <div class="mt-6">
                    <label class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-2">Backup Email Address</label>
                    <input 
                      type="email" 
                      v-model="recoveryEmailInput" 
                      placeholder="e.g., backup@example.com" 
                      class="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent focus:bg-white outline-none transition-all font-medium"
                    />
                  </div>
                  ...
                  <div class="flex flex-col sm:flex-row sm:justify-end gap-3 mt-8">
                    ...
                    <button @click="sendRecoveryOtp" :disabled="isProcessing || !recoveryEmailInput" class="w-full sm:w-auto h-12 px-8 rounded-xl font-bold text-white bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 shadow-lg shadow-indigo-500/30 transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center">
                      <svg v-if="isProcessing" class="w-5 h-5 mr-2 animate-spin text-white" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                ...
                <div v-if="activeModal === 'otp'">
                  <div class="w-14 h-14 bg-indigo-100 text-indigo-600 rounded-2xl flex items-center justify-center mb-5">
                  ...
                  <div class="flex flex-col sm:flex-row sm:justify-end gap-3 mt-8">
                    ...
                    <button @click="verifyRecoveryOtp" :disabled="isProcessing || otpInput.length < 6" class="w-full sm:w-auto h-12 px-8 rounded-xl font-bold text-white bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 shadow-lg shadow-indigo-500/30 transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center">
                      <svg v-if="isProcessing" class="w-5 h-5 mr-2 animate-spin text-white" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                ...
                <div v-if="activeModal === 'security_questions'">
                  <div class="flex justify-between items-center mb-6 pr-8">
                    ...
                    <span class="text-xs font-black text-indigo-700 bg-indigo-100 px-3 py-1.5 rounded-full uppercase tracking-wider">
                      Step {{ currentQuestionIndex + 1 }} / {{ securityQuestionsList.length }}
                    </span>
                  </div>

                  <!-- Neon Glowing Progress Bar -->
                  <div class="w-full bg-gray-100 rounded-full h-2 mb-8 overflow-hidden relative">
                    <div 
                      class="bg-gradient-to-r from-blue-500 to-indigo-500 h-full rounded-full transition-all duration-500 ease-out relative" 
                      :style="{ width: ((currentQuestionIndex + 1) / securityQuestionsList.length * 100) + '%' }"
                    >
                      <div class="absolute top-0 right-0 bottom-0 w-10 bg-white/30 blur-md"></div>
                    </div>
                  </div>
                  ...
                  <div class="min-h-[140px] flex flex-col justify-center">
                    <transition name="slide-fade" mode="out-in">
                      <div :key="currentQuestionIndex" class="space-y-5">
                        <label class="block text-xl font-bold text-gray-800 leading-snug">
                        ...
                        <input 
                          ...
                          class="w-full p-4 text-base font-medium bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent focus:bg-white outline-none transition-all shadow-inner"
                          autofocus
                        />
                      </div>
                    </transition>
                  ...
                  <!-- Wizard Controls -->
                  <div class="flex flex-col-reverse sm:flex-row sm:justify-between mt-10 gap-3 sm:gap-0 border-t border-gray-100 pt-6">
                    <button @click="closeModal" class="w-full sm:w-auto h-12 px-6 rounded-xl font-bold text-gray-500 hover:text-gray-900 transition-colors">
                      Cancel Setup
                    </button>
                    ...
                    <div class="flex gap-3 w-full sm:w-auto justify-end">
                      ...
                      <button 
                        ...
                        class="w-full sm:w-auto h-12 px-8 rounded-xl font-bold text-white bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 shadow-lg shadow-emerald-500/30 transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
                      >
                        <svg v-if="isProcessing" class="w-5 h-5 mr-2 animate-spin text-white" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
...
const fetchSettings = async () => {
  try {
    const response = await api.get('/client/security-settings'); 
    if (response.data) {
...
const confirmStandardToggle = async () => {
  ...
  try {
    if (field === 'security_questions' && value === false) {
      await api.post('/client/security-settings/questions', { action: 'disable' });
    } else {
      ...
      const response = await api.put('/client/security-settings', payload);
...
const sendRecoveryOtp = async () => {
  ...
  try {
    await api.post('/client/security-settings/send-otp', {
      action: action,
...
const verifyRecoveryOtp = async () => {
  isProcessing.value = true;
  try {
    await api.post('/client/security-settings/verify-otp', { otp: otpInput.value });
...
const saveSecurityQuestions = async () => {
  ...
  isProcessing.value = true;
  try {
    await api.post('/client/security-settings/questions', {
      action: 'enable',
