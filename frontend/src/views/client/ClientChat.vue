        ...
        >
          ...
          >
            ...
            <div class="flex flex-col max-w-[95%] sm:max-w-[85%] md:max-w-[75%]" :class="message.sender === 'me' ? 'items-end' : 'items-start'">
              ...
              <div v-else-if="message.type === 'official_deal'" class="bg-gradient-to-br from-indigo-900/50 to-cyan-900/50 border border-cyan-500/50 rounded-xl p-4 shadow-lg shadow-cyan-900/20 w-full max-w-sm sm:max-w-md">
                 ...
                 <div v-else-if="message.sender !== 'me'" class="mt-3 text-center text-xs font-bold" :class="{ 'text-emerald-400': message.payload?.deal_status === 'ongoing', 'text-red-400': message.payload?.deal_status === 'declined' }">
                    <span v-if="message.payload?.deal_status === 'ongoing'">✅ You Accepted this Deal</span>
  ...
  try {
    ...
    if (res.data.success) {
      ...
      messages.value.push({
        id: Date.now(),
        sender: 'me',
        text: action === 'agree' ? "I have agreed to the official deal. Let's begin the ongoing project!" : "I have declined the official deal. Can we adjust the terms?",
        type: 'text',
