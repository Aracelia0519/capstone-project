  ...
  <div class="min-h-screen relative pb-20">
    ...
    <div v-else class="container mx-auto px-4 py-8">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <!-- Left Side: Product Info & Reviews -->
                ...
                <div>
                  ...
                  <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <label class="flex flex-col items-start gap-2 p-3 border-2 rounded-xl transition-all duration-200 relative overflow-hidden" :class="[
                        !isCodAvailable ? 'opacity-50 cursor-not-allowed border-gray-100 bg-gray-50 grayscale' : (paymentMethod === 'cod' ? 'border-green-500 bg-green-50/30 cursor-pointer' : 'border-gray-100 hover:border-gray-200 cursor-pointer')
                      ]">
                    ...
                    <label class="flex flex-col items-start gap-2 p-3 border-2 rounded-xl transition-all duration-200 relative overflow-hidden" :class="[
                        !isGcashAvailable ? 'opacity-50 cursor-not-allowed border-gray-100 bg-gray-50 grayscale' : (paymentMethod === 'gcash' ? 'border-blue-500 bg-blue-50/30 cursor-pointer' : 'border-gray-100 hover:border-gray-200 cursor-pointer')
                      ]">
                    ...
                    <label class="flex flex-col items-start gap-2 p-3 border-2 rounded-xl transition-all duration-200 relative overflow-hidden" :class="[
                        !isPickupAvailable ? 'opacity-50 cursor-not-allowed border-gray-100 bg-gray-50 grayscale' : (paymentMethod === 'pick-up' ? 'border-amber-500 bg-amber-50/30 cursor-pointer' : 'border-gray-100 hover:border-gray-200 cursor-pointer')
                      ]">
                  ...
                  <p v-if="!isCodAvailable || !isGcashAvailable || !isPickupAvailable" class="text-xs text-amber-600 mt-2 italic font-medium">
                    * Some payment/delivery methods are disabled because this seller does not support them.
                  </p>
                </div>
...
const isCodAvailable = computed(() => checkAvailability(selectedProduct.value?.distributor_cod_enabled, true))
const isGcashAvailable = computed(() => checkAvailability(selectedProduct.value?.distributor_gcash_enabled, false))
const isPickupAvailable = computed(() => checkAvailability(selectedProduct.value?.distributor_pickup_enabled, false))
...
const fetchProductDetails = async (id) => {
  try {
    ...
    if (response.data.success) {
      ...
      // Determine default payment method based on availability
...
onMounted(() => {
  if (route.query.order_number) { 
    ...
    fetchProductDetails(route.params.id)
    fetchOtherProducts() // Fetch carousel
  }
})
</script>
