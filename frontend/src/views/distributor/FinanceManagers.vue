  ...
  <div class="p-4 md:p-6 finance-managers">
    ...
    <div class="mb-6 md:mb-8">
      <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          <h1 class="text-2xl md:text-3xl font-bold text-gray-800">Finance Managers</h1>
          <p class="text-gray-600 mt-2">Manage your finance department managers and personnel</p>
        </div>
        <div class="flex items-center space-x-3">
          <!-- Stats Quick View -->
          <div class="flex items-center space-x-4">
            <div class="text-center">
              <div class="text-2xl font-bold text-purple-600">{{ financeStats?.total || 0 }}</div>
              <div class="text-xs text-gray-500">Total Staff</div>
            </div>
            <div class="text-center">
              <div class="text-2xl font-bold text-green-600">{{ financeStats?.active || 0 }}</div>
              <div class="text-xs text-gray-500">Active</div>
            </div>
            <div class="text-center">
              <div class="text-2xl font-bold text-blue-600">{{ financeStats?.with_accounts || 0 }}</div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
      ...
      <div class="lg:col-span-1 space-y-6">
        <!-- Quick Actions Card -->
        <div class="bg-gradient-to-br from-purple-50 to-purple-50 rounded-xl shadow-sm border border-purple-100 p-5">
          ...
          <div class="space-y-3">
            ...
            <button @click="exportToExcel" 
              class="w-full px-4 py-3 bg-white border border-purple-200 text-purple-600 hover:bg-purple-50 rounded-lg transition-colors flex items-center justify-center">
            ...
            <div class="flex">
              ...
              <div>
                ...
                <p class="text-sm text-yellow-700 mt-1">
                  You must be verified as a distributor to manage finance managers.
            ...
            <div class="pt-4 border-t border-gray-200">
              <div class="grid grid-cols-2 gap-3">
                <div class="text-center p-3 bg-purple-50 rounded-lg">
                  <div class="text-xl font-bold text-purple-700">{{ financeStats?.pending || 0 }}</div>
                  <div class="text-xs text-purple-600">Pending Review</div>
                </div>
                <div class="text-center p-3 bg-red-50 rounded-lg">
                  <div class="text-xl font-bold text-red-700">{{ financeStats?.inactive || 0 }}</div>
      ...
      <div class="lg:col-span-3 space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl shadow-lg p-5 text-white">
            <h3 class="font-semibold mb-4">Finance Overview</h3>
            <div class="space-y-4">
              <div>
                <div class="text-3xl font-bold">{{ financeStats?.total || 0 }}</div>
                <div class="text-sm opacity-90">Total Finance Managers</div>
              </div>
              <div>
                <div class="text-2xl font-bold">{{ financeStats?.active || 0 }}</div>
...
export default {
  ...
  data() {
    return {
      ...
      recentActivities: [
        ...
        {
          id: 2,
          message: 'Finance report generated for Q3',
          time: '3 days ago',
        ...
        {
          id: 3,
          message: 'Budget review meeting completed',
          time: '1 week ago',
  ...
  methods: {
    ...
    getVisiblePages() {
      if (!this.financePagination) return []
      
      const current = this.financePagination.current_page
      const last = this.financePagination.last_page
      const pages = []
    ...
    getActiveRate() {
      if (!this.financeStats || this.financeStats.total === 0) return 0
      return Math.round((this.financeStats.active / this.financeStats.total) * 100)
    },

    getAccountRate() {
      if (!this.financeStats || this.financeStats.total === 0) return 0
      return Math.round((this.financeStats.with_accounts / this.financeStats.total) * 100)
    },

    getStatusDistribution() {
      const total = this.financeManagers.length
      if (total === 0) return []
      
      const statuses = ['active', 'pending', 'inactive', 'on_leave']
      
      const distribution = statuses.map(status => {
        const count = this.financeManagers.filter(manager => manager.status === status).length
        return {
