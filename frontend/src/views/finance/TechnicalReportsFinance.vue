    ...
    <div class="max-w-6xl mx-auto">
      ...
      <div class="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 class="text-3xl font-black text-slate-900 tracking-tight flex items-center gap-3">
            <Wrench class="w-8 h-8 text-emerald-600" />
            Technical Support
          </h1>
        ...
        <div class="flex items-center gap-3 bg-slate-100 p-1.5 rounded-xl border border-slate-200">
            <button 
                ...
                :class="['px-4 py-2 rounded-lg text-sm font-bold transition-all', activeTab === 'create' ? 'bg-emerald-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-900']"
            >
                Submit Report
            </button>
            <button 
                @click="activeTab = 'history'" 
                :class="['px-4 py-2 rounded-lg text-sm font-bold transition-all', activeTab === 'history' ? 'bg-emerald-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-900']"
            >
                My Reports
            </button>
        </div>
      ...
      <div v-if="activeTab === 'create'" class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
            ...
            <div v-for="(step, index) in steps" :key="index" class="flex-1 flex items-center relative">
                <div class="flex flex-col items-center relative z-10">
                    <div :class="[
                        ...
                        currentStep > index + 1 ? 'bg-teal-500 text-white ring-teal-100' :
                        currentStep === index + 1 ? 'bg-emerald-600 text-white ring-emerald-100' :
                        'bg-slate-200 text-slate-500 ring-transparent'
                    ]">
                        <Check v-if="currentStep > index + 1" class="w-4 h-4" />
                        <span v-else>{{ index + 1 }}</span>
                    </div>
                    <span :class="['text-[10px] uppercase tracking-wider font-bold mt-2 absolute top-full whitespace-nowrap', currentStep >= index + 1 ? 'text-emerald-600' : 'text-slate-400']">
                        {{ step }}
                    </span>
                </div>
                <div v-if="index < steps.length - 1" :class="[
                    'flex-1 h-1 mx-4 rounded-full transition-colors duration-500',
                    currentStep > index + 1 ? 'bg-teal-500/50' : 'bg-slate-200'
                ]"></div>
            </div>
        </div>
            ...
            <transition name="fade-slide" mode="out-in">
                <div v-if="currentStep === 1" class="space-y-6 max-w-2xl mx-auto">
                    <div>
                        ...
                        <p class="text-sm text-slate-600 mb-6">Help us understand what kind of problem you're facing in the Finance Hub.</p>
                    </div>
                    ...
                    <div class="space-y-4">
                        <div class="space-y-1.5">
                            ...
                            <div class="relative">
                                <select v-model="form.category" class="w-full bg-white border border-slate-300 text-slate-900 rounded-xl px-4 py-3 pr-10 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all appearance-none">
                                    ...
                                    <option value="payment_issue">Payment / Disbursement Issue</option>
                                    <option value="order_issue">Invoice / Order Issue</option>
                        ...
                        <div class="space-y-1.5">
                            ...
                            <div class="relative">
                                <select v-model="form.page" class="w-full bg-white border border-slate-300 text-slate-900 rounded-xl px-4 py-3 pr-10 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all appearance-none">
                                    ...
                                    <option value="Budget Release">Budget Release</option>
                                    <option value="Payroll Request">Payroll Request</option>
                                    <option value="Payroll Paid">Payroll Paid</option>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-1.5">
                            <label class="text-xs font-bold text-slate-700 uppercase tracking-widest flex items-center gap-2">
                                <Smartphone class="w-4 h-4 text-emerald-600" /> Device Type <span class="text-red-500">*</span>
                            </label>
                            <input v-model="form.device" type="text" placeholder="e.g. Windows PC, iPhone 13, Mac" class="w-full bg-white border border-slate-300 text-slate-900 rounded-xl px-4 py-3 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all">
                        </div>

                        <div class="space-y-1.5">
                            <label class="text-xs font-bold text-slate-700 uppercase tracking-widest flex items-center gap-2">
                                <Globe class="w-4 h-4 text-teal-500" /> Browser <span class="text-red-500">*</span>
                            </label>
                            <input v-model="form.browser" type="text" placeholder="e.g. Chrome, Safari, Firefox" class="w-full bg-white border border-slate-300 text-slate-900 rounded-xl px-4 py-3 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all">
                        </div>
                    </div>
                </div>
                        ...
                        <div class="space-y-1.5">
                            ...
                            <div class="w-full border-2 border-dashed border-slate-300 rounded-xl p-4 flex flex-col items-center justify-center bg-slate-50 hover:bg-slate-100 transition-colors relative group">
                                ...
                                <div v-if="!attachmentPreview" class="flex flex-col items-center">
                                    <UploadCloud class="w-8 h-8 text-emerald-500 mb-2 group-hover:scale-110 transition-transform" />
        ...
        <div class="px-6 py-4 bg-slate-50 border-t border-slate-200 flex items-center justify-between">
            ...
            <button 
                ...
                class="px-8 py-2.5 rounded-xl font-bold transition-all flex items-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-white disabled:opacity-50 disabled:cursor-not-allowed shadow-md shadow-emerald-500/20"
            >
            ...
            <button 
                ...
                class="px-8 py-2.5 rounded-xl font-black transition-all flex items-center gap-2 bg-teal-500 hover:bg-teal-400 text-white disabled:opacity-50 shadow-md shadow-teal-500/20"
            >
      ...
      <div v-if="activeTab === 'history'" class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden p-6 min-h-[500px]">
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div v-for="report in reports" :key="report.id" class="bg-white border border-slate-200 rounded-xl p-5 hover:border-emerald-400 hover:shadow-md transition-all">
                <div class="flex justify-between items-start mb-3">
                    <div>
                        <Badge variant="outline" :class="[
                            'text-[9px] font-black uppercase tracking-widest px-2 py-0.5 border-none',
                            report.status === 'reviewed' ? 'bg-teal-100 text-teal-600' : 'bg-amber-100 text-amber-600'
                        ]">
                            {{ report.status }}
                        </Badge>
                ...
                <div class="flex items-center justify-between text-xs border-t border-slate-100 pt-3">
                    <div class="flex items-center gap-1.5 text-slate-600">
                        <MapPin class="w-3.5 h-3.5 text-emerald-500" /> {{ report.page }}
                    </div>
                    <div v-if="report.attachment" class="flex items-center gap-1 text-emerald-600 font-medium cursor-pointer hover:underline" @click="viewImage(report.attachment)">
...
const submitReport = async () => {
    ...
    try {
        ...
        const response = await api.post('/finance/technical-reports', formData, {
...
const fetchMyReports = async () => {
    ...
    try {
        const response = await api.get('/finance/technical-reports')
        if (response.data.success) {
