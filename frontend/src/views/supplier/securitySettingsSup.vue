...
const fetchSettings = async () => {
  try {
    const response = await api.get('/supplier/security-settings'); 
    if (response.data) {
...
const sendRecoveryOtp = async () => {
  ...
  try {
    await api.post('/supplier/security-settings/send-otp', {
      action: action,
...
const verifyRecoveryOtp = async () => {
  isProcessing.value = true;
  try {
    await api.post('/supplier/security-settings/verify-otp', { otp: otpInput.value });
...
const saveSecurityQuestions = async () => {
  ...
  isProcessing.value = true;
  try {
    await api.post('/supplier/security-settings/questions', {
      action: 'enable',
