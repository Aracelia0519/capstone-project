<template>
  <div class="p-4 md:p-8 font-sans selection:bg-emerald-100 selection:text-emerald-900">
    <div class="max-w-8xl mx-auto space-y-6">
      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-200 dark:border-slate-800">
        <div class="flex items-center gap-5">
          <div class="w-14 h-14 rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-600 text-white flex items-center justify-center shrink-0 shadow-lg shadow-emerald-500/30">
            <Bell class="w-7 h-7" />
          </div>
          <div>
            <h1 class="text-2xl md:text-3xl font-black text-slate-900 dark:text-white tracking-tight">System Alerts</h1>
            <p class="text-sm text-slate-500 mt-1 font-medium">
              You have <strong class="text-emerald-600 dark:text-emerald-400">{{ unreadCount }}</strong> unread messages.
            </p>
          </div>
        </div>
        <Button 
      ...
      <div class="bg-white dark:bg-slate-900 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-200 dark:border-slate-800 overflow-hidden relative">
        ...
        <div v-if="isLoading" class="p-24 flex flex-col items-center justify-center text-slate-400">
          <Loader2 class="w-12 h-12 animate-spin mb-4 text-emerald-500" />
        ...
        <div v-else class="divide-y divide-slate-100 dark:divide-slate-800/50">
          <div 
            ...
            :class="!notification.is_read ? 'bg-emerald-50/50 dark:bg-emerald-900/10' : ''"
            @click="markAsRead(notification)"
          >
            <!-- Left border indicator for unread -->
            <div 
              v-if="!notification.is_read" 
              class="absolute left-0 top-0 bottom-0 w-1.5 bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]"
            ></div>
            ...
            <div class="flex-1 space-y-2 min-w-0">
              <div class="flex flex-col md:flex-row md:items-start justify-between gap-2">
                ...
                <div class="flex items-center gap-2 shrink-0 md:pt-1">
                  <span class="text-xs font-bold uppercase tracking-wider" :class="!notification.is_read ? 'text-emerald-600' : 'text-slate-400'">
                    {{ formatTimeAgo(notification.created_at) }}
                  </span>
                  <span v-if="!notification.is_read" class="w-2.5 h-2.5 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.6)]"></span>
                </div>
              </div>
              ...
              <div v-if="notification.attachment || notification.attachment_url" 
                   ...
                   class="mt-4 inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white dark:bg-slate-900 text-xs font-bold text-emerald-600 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-900 shadow-sm transition-transform group-hover:scale-105 cursor-pointer hover:bg-emerald-50">
...
const markAsRead = async (notification) => {
  ...
  try {
    await api.put(`/supplier/notifications/${notification.id}/read`)
  } catch (error) {
    // Revert if failed
...
const markAllAsRead = async () => {
  ...
  try {
    await api.post('/supplier/notifications/mark-all-read')
  } catch (error) {
