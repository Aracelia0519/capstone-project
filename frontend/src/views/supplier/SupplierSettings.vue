  ...
  <div class="page-transition p-4 md:p-6 min-h-screen">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
              ...
              <div class="flex flex-col md:flex-row md:items-center gap-4 md:gap-6">
                ...
                <div class="flex-1 space-y-2">
                  ...
                  <div class="flex flex-wrap gap-2">
                    ...
                    <Badge v-if="userInfo.role === 'supplier' && verificationData" 
                      :class="[
                        'px-3 py-1.5 rounded-full text-sm font-medium shadow-sm border',
      ...
      <div class="lg:col-span-1 space-y-5">
        ...
        <Card v-if="userInfo.role === 'supplier'" class="card-shadow depth-hover border-gray-100 overflow-hidden rounded-2xl">
          <div class="bg-linear-to-r from-green-50 to-emerald-50 px-6 py-4 border-b border-gray-200 card-header-hover">
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-3">
                ...
                <h2 class="text-lg font-bold text-gray-800">Supplier Info</h2>
              </div>
          ...
          <CardContent class="p-6 space-y-6">
            <div class="grid grid-cols-1 gap-6">
              <div class="form-group space-y-2">
                ...
                <Input v-model="supplierInfo.company_name" type="text" 
              ...
              <div class="form-group space-y-2">
                ...
                <Input v-model="supplierInfo.business_registration_number" type="text" 
                  class="h-12 bg-gray-50 border-gray-200 rounded-xl focus-visible:ring-green-500 focus-visible:border-green-500 transition-all duration-300 form-input"
                  :readonly="verificationData && verificationData.has_submitted" />
              </div>
            </div>
            ...
            <div class="grid grid-cols-1 gap-6">
              <div class="form-group space-y-2">
                ...
                <Input v-model="supplierInfo.valid_id_type_display" type="text" 
              ...
              <div class="form-group space-y-2">
                ...
                <Input v-model="supplierInfo.id_number" type="text" 
                  class="h-12 bg-gray-50 border-gray-200 rounded-xl focus-visible:ring-green-500 focus-visible:border-green-500 transition-all duration-300 form-input"
                  :readonly="verificationData && verificationData.has_submitted" />
              </div>
            </div>
              ...
              <div class="flex items-start">
                ...
                <div>
                  ...
                  <p class="text-xs text-gray-600">
                    Your supplier information is currently locked because you have submitted business verification.
                  </p>
                </div>
              </div>
            </div>
              ...
              <Button @click="saveSupplierInfo" 
                ...
                :class="[
                  supplierInfoChanged && !savingSupplier && (!verificationData || !verificationData.has_submitted)
                    ...
                    : 'bg-gray-200 text-gray-500 hover:bg-gray-200 cursor-not-allowed',
                  savingSupplier ? 'bg-linear-to-r from-green-400 to-green-500 cursor-wait' : ''
                ]">
                <Loader2 v-if="savingSupplier" class="w-5 h-5 mr-2 animate-spin" />
                <Save v-else class="w-5 h-5 mr-2 icon-hover" />
                {{ savingSupplier ? 'Saving...' : 'Update' }}
              </Button>
            </div>
          </CardContent>
        </Card>
      ...
      <div v-if="userInfo.role === 'supplier'" class="lg:col-span-3">
...
const supplierInfoChanged = computed(() => JSON.stringify(supplierInfo) !== JSON.stringify(originalSupplierInfo.value))
const hasUnsavedChanges = computed(() => userInfoChanged.value || supplierInfoChanged.value)
...
const getCurrentLocation = async () => {
  ...
  navigator.geolocation.getCurrentPosition(
    async (position) => {
      ...
      // Enforce strict geo-check against Cavite Bounds before processing
...
const setOriginalData = () => {
  ...
  originalSupplierInfo.value = JSON.parse(JSON.stringify(supplierInfo))
}
...
const saveSupplierInfo = async () => {
  if (!supplierInfoChanged.value || savingSupplier.value) return
  
  savingSupplier.value = true
  try {
    const response = await axios.put('/profile/supplier', {
      company_name: supplierInfo.company_name,
      business_registration_number: supplierInfo.business_registration_number,
      valid_id_type: supplierInfo.valid_id_type,
      id_number: supplierInfo.id_number
    })
    ...
    if (error.response && error.response.status === 401) {
      ...
      toast.error(error.response?.data?.message || 'Failed to update supplier information. Please try again.')
      Object.assign(supplierInfo, JSON.parse(JSON.stringify(originalSupplierInfo.value)))
    }
  } finally {
