  ...
  <div class="p-6 md:p-8 max-w-7xl mx-auto space-y-8 min-h-screen rounded-3xl">
      ...
      >
        ...
        <div class="p-6">
          ...
          <div class="mt-4 flex flex-wrap gap-2">
            <a v-if="vehicle.cr_file_path" :href="getFileUrl(vehicle.cr_file_path)" target="_blank" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-50 hover:bg-emerald-100 text-emerald-700 text-xs font-bold transition-colors">
              <FileText class="w-3.5 h-3.5" /> CR
            </a>
            <a v-if="vehicle.or_file_path" :href="getFileUrl(vehicle.or_file_path)" target="_blank" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-50 hover:bg-emerald-100 text-emerald-700 text-xs font-bold transition-colors">
              <FileText class="w-3.5 h-3.5" /> OR
            </a>
            <a v-if="vehicle.proof_of_ownership_path" :href="getFileUrl(vehicle.proof_of_ownership_path)" target="_blank" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-50 hover:bg-emerald-100 text-emerald-700 text-xs font-bold transition-colors">
    ...
    <Dialog :open="showAddModal" @update:open="showAddModal = $event">
      <DialogContent class="sm:max-w-[600px] rounded-[2rem] border-slate-100 shadow-2xl p-0 overflow-hidden" @interact-outside="e => e.preventDefault()">
        <div class="bg-gradient-to-r from-emerald-500 to-teal-500 p-6 text-white">
          ...
          <p class="text-emerald-50 text-sm mt-1 opacity-90">Step {{ currentStep }} of {{ totalSteps }}: {{ stepTitles[currentStep - 1] }}</p>
        </div>
        ...
        <div class="p-6 bg-white min-h-[380px]">
          ...
          <div class="flex justify-between items-center mb-8 relative">
            ...
            <div class="absolute left-0 top-1/2 -translate-y-1/2 h-1 bg-emerald-500 rounded-full z-0 transition-all duration-300" :style="{ width: `${((currentStep - 1) / (totalSteps - 1)) * 100}%` }"></div>
            ...
            <div v-for="step in totalSteps" :key="step" class="relative z-10 flex flex-col items-center justify-center">
              <div :class="['w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm shadow-sm transition-colors duration-300', 
                  ...
                  (currentStep > step ? 'bg-emerald-500 text-white' : 'bg-white text-slate-400 border border-slate-200')]">
          ...
          <div v-if="currentStep === 1" class="grid gap-5 animate-in fade-in slide-in-from-right-4">
            <div class="grid grid-cols-2 gap-4">
              ...
              <div class="space-y-2">
                ...
                <select id="makeSelection" v-model="makeSelection" class="w-full h-11 px-3 rounded-xl border border-slate-200 text-sm font-medium focus:ring-2 focus:ring-emerald-500 focus:outline-none appearance-none bg-slate-50 focus:bg-white transition-colors">
          ...
          <div v-if="currentStep === 2" class="grid gap-5 animate-in fade-in slide-in-from-right-4">
            ...
            <div class="grid grid-cols-2 gap-4">
              ...
              <div class="space-y-2">
                ...
                <select id="fuel_type" v-model="newVehicle.fuel_type" class="w-full h-11 px-3 rounded-xl border border-slate-200 text-sm font-medium focus:ring-2 focus:ring-emerald-500 focus:outline-none appearance-none bg-slate-50 focus:bg-white transition-colors">
          ...
          <div v-if="currentStep === 3" class="grid gap-6 animate-in fade-in slide-in-from-right-4">
            <div class="p-6 bg-emerald-50/50 rounded-[2rem] border border-emerald-100/50 flex flex-col gap-6">
              <div class="space-y-2">
              ...
              <div class="space-y-2">
                <Label for="paintCapacity" class="text-xs font-bold text-emerald-700 uppercase tracking-wider">Paint Capacity (kg) <span class="text-red-500">*</span></Label>
                <Input id="paintCapacity" type="number" v-model="newVehicle.paint_capacity" placeholder="e.g. 1000" class="rounded-xl h-12 bg-white border-slate-200 shadow-sm ring-offset-emerald-50 focus-visible:ring-emerald-500 text-lg font-bold" />
            ...
            <div class="space-y-5 p-1">
              <div class="space-y-2 group">
                <div class="flex justify-between items-center mb-1">
                  ...
                  <a v-if="isEditing && newVehicle.cr_file_path" :href="getFileUrl(newVehicle.cr_file_path)" target="_blank" class="text-[10px] text-emerald-600 hover:text-emerald-800 font-bold flex items-center gap-1 transition-colors">
                    <ExternalLink class="w-3 h-3" /> View Current
                  </a>
                </div>
                <Input type="file" accept="image/*,.pdf" @change="e => handleFileChange(e, 'cr_file')" class="cursor-pointer file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-emerald-50 file:text-emerald-700 group-hover:file:bg-emerald-100 h-auto py-2 px-3 bg-slate-50 border-dashed border-2 transition-colors" />
              </div>
              
              <div class="space-y-2 group">
                <div class="flex justify-between items-center mb-1">
                  <Label class="text-xs font-bold text-slate-600 uppercase tracking-wider block">Official Receipt (OR)</Label>
                  <a v-if="isEditing && newVehicle.or_file_path" :href="getFileUrl(newVehicle.or_file_path)" target="_blank" class="text-[10px] text-emerald-600 hover:text-emerald-800 font-bold flex items-center gap-1 transition-colors">
                    <ExternalLink class="w-3 h-3" /> View Current
                  </a>
                </div>
              ...
              <div class="space-y-2 group">
                <div class="flex justify-between items-center mb-1">
                  ...
                  <a v-if="isEditing && newVehicle.proof_of_ownership_path" :href="getFileUrl(newVehicle.proof_of_ownership_path)" target="_blank" class="text-[10px] text-emerald-600 hover:text-emerald-800 font-bold flex items-center gap-1 transition-colors">
                    <ExternalLink class="w-3 h-3" /> View Current
                  </a>
                </div>
        ...
        <div class="p-6 bg-slate-50 border-t border-slate-100 flex gap-3">
          ...
          <Button v-else class="flex-1 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white h-11 shadow-lg shadow-emerald-500/20 font-bold" @click="promptSave">
    ...
    <AlertDialog :open="showSaveDialog" @update:open="showSaveDialog = $event">
      <AlertDialogContent class="rounded-[2rem] bg-white border border-slate-100 shadow-2xl p-8">
        <div class="w-16 h-16 rounded-full bg-emerald-50 flex items-center justify-center mb-6 mx-auto">
          <Save class="w-8 h-8 text-emerald-500" />
        </div>
        ...
        <AlertDialogFooter class="mt-8 flex gap-3 sm:justify-center">
          ...
          <AlertDialogAction class="flex-1 rounded-xl bg-emerald-600 text-white hover:bg-emerald-700 h-12 shadow-lg shadow-emerald-500/20 font-bold" :disabled="submitting" @click="confirmSave">
...
const newVehicle = ref({ 
  ...
  type: 'Truck', status: 'Active', max_weight: null, paint_capacity: null 
})
...
const confirmSave = async () => {
  try {
    ...
    if (isEditing.value) {
      response = await api.post(`/supplier/vehicles/update/${selectedVehicleId.value}`, formData, config)
    } else {
      response = await api.post('/supplier/vehicles', formData, config)
    }
