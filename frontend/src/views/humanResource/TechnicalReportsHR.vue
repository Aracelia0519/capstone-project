    ...
    <div class="max-w-6xl mx-auto">
      ...
      <div class="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div class="flex items-center gap-3 bg-slate-100 p-1.5 rounded-xl border border-slate-200">
            <button 
                ...
                :class="['px-4 py-2 rounded-lg text-sm font-bold transition-all', activeTab === 'create' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-900']"
            >
                Submit Report
            </button>
            <button 
                @click="activeTab = 'history'" 
                :class="['px-4 py-2 rounded-lg text-sm font-bold transition-all', activeTab === 'history' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-900']"
            >
                My Reports
            </button>
        </div>
      ...
      <div v-if="activeTab === 'create'" class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
            ...
            <div v-for="(step, index) in steps" :key="index" class="flex-1 flex items-center relative">
                <div class="flex flex-col items-center relative z-10">
                    <div :class="[
                        ...
                        currentStep === index + 1 ? 'bg-indigo-600 text-white ring-indigo-100' :
                        'bg-slate-200 text-slate-500 ring-transparent'
                    ]">
                        <Check v-if="currentStep > index + 1" class="w-4 h-4" />
                        <span v-else>{{ index + 1 }}</span>
                    </div>
                    <span :class="['text-[10px] uppercase tracking-wider font-bold mt-2 absolute top-full whitespace-nowrap', currentStep >= index + 1 ? 'text-indigo-600' : 'text-slate-400']">
                        {{ step }}
                    </span>
                </div>
            ...
            <transition name="fade-slide" mode="out-in">
                <div v-if="currentStep === 1" class="space-y-6 max-w-2xl mx-auto">
                    <div>
                        ...
                        <p class="text-sm text-slate-600 mb-6">Help us understand what kind of problem you're facing in the HR Hub.</p>
                    </div>
                    ...
                    <div class="space-y-4">
                        <div class="space-y-1.5">
                            ...
                            <div class="relative">
                                <select v-model="form.category" class="w-full bg-white border border-slate-300 text-slate-900 rounded-xl px-4 py-3 pr-10 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all appearance-none">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-1.5">
                            <label class="text-xs font-bold text-slate-700 uppercase tracking-widest flex items-center gap-2">
                                <Smartphone class="w-4 h-4 text-indigo-600" /> Device Type <span class="text-red-500">*</span>
                            </label>
                            <input v-model="form.device" type="text" placeholder="e.g. Windows PC, iPhone 13, Mac" class="w-full bg-white border border-slate-300 text-slate-900 rounded-xl px-4 py-3 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all">
                        </div>
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input v-model="form.browser" type="text" placeholder="e.g. Chrome, Safari, Firefox" class="w-full bg-white border border-slate-300 text-slate-900 rounded-xl px-4 py-3 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all">
                        </div>
                    </div>
                </div>
                        ...
                        <div class="space-y-1.5">
                            ...
                            <div class="w-full border-2 border-dashed border-slate-300 rounded-xl p-4 flex flex-col items-center justify-center bg-slate-50 hover:bg-slate-100 transition-colors relative group">
                                ...
                                <div v-if="!attachmentPreview" class="flex flex-col items-center">
                                    <UploadCloud class="w-8 h-8 text-indigo-500 mb-2 group-hover:scale-110 transition-transform" />
        ...
        <div class="px-6 py-4 bg-slate-50 border-t border-slate-200 flex items-center justify-between">
            ...
            <button 
                ...
                class="px-8 py-2.5 rounded-xl font-bold transition-all flex items-center gap-2 bg-indigo-600 hover:bg-indigo-500 text-white disabled:opacity-50 disabled:cursor-not-allowed shadow-md shadow-indigo-500/20"
            >
      ...
      <div v-if="activeTab === 'history'" class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden p-6 min-h-[500px]">
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div v-for="report in reports" :key="report.id" class="bg-white border border-slate-200 rounded-xl p-5 hover:border-indigo-400 hover:shadow-md transition-all">
                ...
                <div class="flex items-center justify-between text-xs border-t border-slate-100 pt-3">
                    <div class="flex items-center gap-1.5 text-slate-600">
                        <MapPin class="w-3.5 h-3.5 text-indigo-500" /> {{ report.page }}
                    </div>
                    <div v-if="report.attachment" class="flex items-center gap-1 text-indigo-600 font-medium cursor-pointer hover:underline" @click="viewImage(report.attachment)">
...
const submitReport = async () => {
    ...
    try {
        ...
        const response = await api.post('/hr/technical-reports', formData, {
...
const fetchMyReports = async () => {
    ...
    try {
        const response = await api.get('/hr/technical-reports')
        if (response.data.success) {
