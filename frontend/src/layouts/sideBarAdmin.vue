<template>
  <Sidebar collapsible="icon" class="border-r border-slate-800/50 bg-slate-900 transition-all duration-500 ease-in-out">
    <SidebarHeader class="h-24 border-b border-slate-800/50 flex flex-row items-center px-4 overflow-hidden bg-slate-900">
      <div class="flex items-center gap-3 w-full">
        <div class="relative shrink-0 flex items-center justify-center">
          <Avatar class="w-10 h-10 ring-2 ring-blue-500/30 ring-offset-2 ring-offset-slate-900">
            <div class="w-full h-full bg-gradient-to-br from-blue-600 via-purple-500 to-pink-500 flex items-center justify-center">
        ...
        <div v-if="state === 'expanded' || isMobile" class="flex flex-col min-w-0 nav-text-clip flex-1">
          ...
          <p class="text-[10px] font-semibold text-blue-400 uppercase tracking-widest">System Admin</p>
        </div>
      </div>
    </SidebarHeader>
            ...
            >
              <router-link :to="item.path" class="flex items-center w-full px-2">
                ...
                <Badge 
                  ...
                  variant="outline" 
                  class="ml-auto text-[9px] bg-blue-500/20 text-blue-300 border-blue-500/30 font-bold px-1.5 h-4"
                >
                  {{ item.badge }}
                </Badge>
              </router-link>
    ...
    <SidebarFooter class="px-3 py-4 border-t border-slate-800/50 bg-slate-900 space-y-1">
      <SidebarMenu>
        <SidebarMenuItem>
          <SidebarMenuButton as-child tooltip="System Settings" class="h-11 w-full rounded-xl text-white/70 hover:text-white hover:bg-slate-800/50">
            <router-link to="/admin/SystemSettings" class="flex items-center w-full px-2">
              <div class="shrink-0 flex items-center justify-center w-6 h-6">
    ...
    <Dialog :open="showLogoutModal && !isLoggingOut" @update:open="showLogoutModal = $event">
      <DialogContent class="bg-slate-900/95 backdrop-blur-sm border-slate-800/50 text-white shadow-2xl max-w-[400px] sm:rounded-3xl p-6">
        <div class="flex flex-col items-center">
          ...
          <p class="text-slate-400 mt-2 text-center">Are you sure you want to exit the Admin panel?</p>
          <div class="flex w-full gap-3 mt-8">
            ...
            <Button class="flex-1 rounded-xl bg-gradient-to-r from-blue-500 to-purple-600 hover:opacity-90 transition-opacity" @click="confirmLogout" :disabled="isLoggingOut">
